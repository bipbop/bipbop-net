...
namespace BipbopNet.Tests
{
    public class JuristekTests
    {
        ...
        private readonly Lazy<Juristek.Client> _juristekClient = new Lazy<Juristek.Client>(() => new Juristek.Client(new Client(Environment.GetEnvironmentVariable("BIPBOP_APIKEY"))));
        ...
        public async Task OabTest()
        {
            ...
            foreach (var oabPush in oab.Pushes)
            {
                Assert.IsNotNull(await JuristekClient.Push.Status(oabPush));
            }
        ...
        public async Task HttpServer()
        {
            ...
            var pushConfiguration = await JuristekClient.Push.Create(JuristekClient.CreatePushConfiguration(cnjQuery, new PushConfiguration
            {
                MaxVersion = 1,
        ...
        {
            var cnjQuery = Query.Cnj("0016306-32.2019.8.26.0502");
            Assert.AreEqual(cnjQuery.ToString(), "SELECT FROM 'CNJ'.'PROCESSO' WHERE 'NUMERO_PROCESSO' = '0016306-32.2019.8.26.0502' AND 'UPLOAD' = 'FALSE'");
        }
        ...
        public async Task PushTest()
        {
            ...
            BipbopDocument? document = await webSocket.WaitPush(pushStatus.Push);
        ...
        public async Task RealtimeQuery()
        {
            ...
            var tjspDatabase = description.Databases.First(database => String.Compare(database.Name, "TJSP", StringComparison.OrdinalIgnoreCase) == 0);
            var tjspTable = tjspDatabase.Tables.First(table => String.Compare(table.Name, "PrimeiraInstancia", StringComparison.OrdinalIgnoreCase) == 0);
